...
def render(video, out, track, stamp_img, args, sface=None, limit_frames=None):
    ...
    while True:
        ...
        # 大きさを段階に丸めてリサイズ結果を使い回す（速度＆サイズのチラつき防止）。
        # 刻みが細かいとフレームごとに拡大し直しになるので、目に見えない幅で粗くする
        step = max(2, int(target_w * 0.01) // 2 * 2)
        qw = int(round(target_w / step)) * step
        deg = 0.0
        ...
        if key not in cache:
            ...
            # INTER_AREA は縮小専用。拡大に使うと最近傍と同じ荒い絵になるので分ける
            # （拡大側は LANCZOS4 だと3倍遅いわりに見た目が変わらないので CUBIC）
            interp = cv2.INTER_AREA if qw <= sw else cv2.INTER_CUBIC
            cache[key] = cv2.resize(base, (max(2, qw), nh), interpolation=interp)
            if len(cache) > 400:
                cache.clear()
        st = cache[key]
        sh, sw = st.shape[:2]
...
def main():
    ...
    ap.add_argument("--encoder", choices=["auto", "x264", "videotoolbox"], default="auto",
                    help="auto=Macならハードウェア(videotoolbox)、それ以外はx264")
    ...
    if args.encoder == "auto":
        # Macの内蔵エンコーダはx264より2倍速い。古いMacほど差が出る
        args.encoder = "videotoolbox" if (
            sys.platform == "darwin" and has_encoder("h264_videotoolbox")) else "x264"
    ...
    # スタンプの解像度が足りているかを先に見る（足りないとぼやけた絵で焼き上がる）
    need = max(p["w"] for p in track) * args.scale / sface["w"]
    have = stamp.shape[1]
    if need > have * 1.05:
        print(f"[facestamp] 注意: スタンプ画像の解像度が足りません。"
              f"この動画では最大 {need:.0f}px で描きますが、用意されているのは {have}px です"
              f"（{need / have:.1f}倍に引き伸ばすので輪郭がぼやけます）。"
              f"横 {int(need / 100 + 1) * 100}px 以上の絵に差し替えてください", file=sys.stderr)
