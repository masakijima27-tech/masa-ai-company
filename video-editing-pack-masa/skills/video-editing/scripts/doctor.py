...
def main():
    ...
    # 顔スタンプは同梱されている版だけチェックする
    face_stamp = Path(__file__).resolve().parents[2] / "face-stamp"
    if (face_stamp / "scripts" / "facestamp.py").exists():
        venv_py = face_stamp / ".venv" / "bin" / "python"
        model = face_stamp / "assets" / "face_detection_yunet_2023mar.onnx"
        ok &= check("顔スタンプの環境", venv_py.exists() and model.exists(),
                    f"1回だけ実行してください: bash {face_stamp / 'scripts' / 'setup_env.sh'}")
