...
FILLER_STANDARD = re.compile(r"^(え[えぇー]+と?|えっと+|あ[あぁー]+う?|う[うー]+ん+|ん[んー]+と?)$")
FILLER_STRONG = re.compile(r"^(え[えぇー]+と?|えっと+|あ[あぁー]+う?|う[うー]+ん+|ん[んー]+と?|あの+|まあ+|まぁ+|なんか)$")

# 判定前に落とす記号。長音符（ー）は残す
FILLER_TRIM = re.compile(r"^[\s　「『]+|[\s　、。，．,.…!！?？」』]*$")


def normalize_filler(text):
    """フィラー判定用にゆらぎを吸収する。

    Whisperは「えーー、」「エーー」のように、句読点付きやカタカナで書くことがある。
    素の文字列のまま照合すると一致しないので、判定の直前だけ正規化する。
    長音符は意味を持つので、末尾の記号を落とすときも残す。
    """
    # カタカナ -> ひらがな（長音符ーは変換対象外なのでそのまま残る）
    text = "".join(chr(ord(c) - 0x60) if "ァ" <= c <= "ン" else c for c in text)
    return FILLER_TRIM.sub("", text)
...
def detect_fillers(words, level):
    ...
    while i < n:
        ...
        for k in (3, 2, 1):
            if i + k <= n:
                joined = normalize_filler("".join(w["w"] for w in words[i:i + k]))
                if pat.match(joined):
                    matched = k
                    break
        if matched:
