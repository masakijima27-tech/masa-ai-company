...
## STEP 2.5. 顔スタンプの準備（1回だけ）

顔にスタンプを乗せる機能は、顔を見つけるための部品を別に入れます。
解凍したフォルダで、次の1行を実行してください。

```bash
bash skills/face-stamp/scripts/setup_env.sh
```

20〜30秒で終わります。最後に `セットアップ完了。` と出れば成功です。

- このパック専用の場所に入るので、パソコンの他の設定には触れません
- macOSのバージョンによって入れるものが変わりますが、スクリプトが自動で判断します
- 失敗したときは、画面に「代わりにこれを試してください」というコマンドが出るので、それを実行してください
...
3つ目に **「顔を隠しますか？」** と聞かれます。隠す方を選ぶと、
カット・テロップに続けて顔スタンプまで一気に入ります。大きさの指定はいりません。

顔がスタンプからはみ出そうな箇所が1コマでもあると、そこで止まって完成させません。
確認用の画像が出るので、それを見て「もう少し大きく」と伝えてください。
...
| `face-stamp の環境が未作成です` | STEP 2.5 をやっていません。戻ってください |
| スタンプが小さくて顔が出る | 「もう少し大きく」と伝えてください（数値は自動で上げます） |
| 顔が映っていない場面にスタンプが残る | 「顔がない場面では消して」と伝えてください |
